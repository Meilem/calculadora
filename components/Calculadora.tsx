...
// ==========================================
// 2. DADOS CONSTANTES (EXCETO PLANOS DE EMAIL)
// ==========================================

// TABELAS DE SUGESTÃO PARA PLANOS DE EMAIL (P1 a P7)
const VALORES_BASE_EMAIL = [
  // P1: 2GB + 30 dias arquivamento
  [
...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  <div className="relative group inline-flex items-center ml-1 z-20">
    <span className="text-[#4254a6] dark:text-[#8aa3ff] font-bold text-xs border-[1.5px] border-[#4254a6] dark:border-[#8aa3ff] rounded-full w-4 h-4 flex items-center justify-center cursor-help transition-colors group-hover:bg-[#4254a6]/50 dark:group-hover:bg-[#8aa3ff]/50 group-hover:text-white">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 640 640"
        className="fill-[#4254a6] dark:fill-[#8aa3ff] group-hover:fill-white"
      >
    ...
    <div
      className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-white dark:bg-slate-800 text-gray-800 dark:text-slate-200 text-sm rounded-lg border border-gray-200 dark:border-slate-700 p-4 transform transition-all shadow-lg ${large ? "w-120" : "w-[320px]"}`}
    >
      {title && (
        <div className="font-bold text-gray-900 dark:text-white mb-3 border-b dark:border-slate-700 pb-2">
          {title}
        </div>
      )}
      <div className="max-h-75 overflow-y-auto overflow-x-hidden pr-2 custom-scrollbar">
        {children}
      </div>
...
export default function Calculadora({
  ...
  const calculations = useMemo(() => {
    let consumido = 0;
    let sugeridoBase = 0; // Para as Box
    let sugeridoEmailExtras = 0; // Novo cálculo de e-mail p1 a p7
    let sugeridoSmtp = 0;
    let sugeridoMkt = 0;
    ...
    // === CÁLCULOS DO E-MAIL ===
    if (emailAccounts > 0) {
      // 1. Custo consumido normal do E-mail
      const emailCost =
    ...
    if (extras.hospedagem) {
      ...
      sugeridoEmailExtras += 29.9; // <-- Preço de Venda Sugerido para o Site (Pode ser ajustado)
    }
    if (extras.lgpd && emailAccounts > 0) {
      const lgpdCost = emailAccounts * 1.0;
      consumido += lgpdCost;
      itemsResumo.push({ name: "LGPD", cost: lgpdCost });
      sugeridoEmailExtras += emailAccounts * 3.0; // <-- Preço de Venda Sugerido (Ex: R$ 3 por conta)
    }
    ...
    selectedSmtp,
    selectedMkt,
    selectedEmailPlan, // AQUI GARANTE A CORREÇÃO DO ERRO DO COMPILADOR
  ]);
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f6] dark:bg-slate-900 text-gray-900 dark:text-slate-100 flex flex-col items-center pt-0 font-sans transition-colors duration-300">
      {/* CABEÇALHO */}
      <div className="w-full bg-white dark:bg-[#030f26] text-gray-900 dark:text-white py-2 px-8 mb-8 flex items-center justify-between border-b border-gray-200 dark:border-gray-800 shadow-sm dark:shadow-md transition-colors duration-300">
        <div className="flex flex-row items-center gap-3">
          ...
          <h1 className="text-base font-bold flex items-center">
            Calculadora <span className="font-light ml-1">MHP</span>
            {tituloRota && (
              <span className="ml-3 text-[10px] bg-gray-100 dark:bg-white/20 text-gray-600 dark:text-gray-200 px-2 py-0.5 rounded-full uppercase tracking-wider transition-colors">
                {tituloRota}
              </span>
            )}
          </h1>
        ...
        <button
          ...
          className="p-2 rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-white/10 dark:hover:bg-white/20 transition-colors duration-300"
          title={
